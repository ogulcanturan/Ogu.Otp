...
namespace Ogu.Otp
{
    public static class Base32Helper
    {
        ...
        public static string GenerateBase32(HashAlgorithmKind hashingAlgorithm = HashAlgorithmKind.Sha1)
        {
            var length = (sbyte)hashingAlgorithm;

            return string.Create((length + 4) / 5 * 8, 0, (buffer, _) =>
            {
        ...
        public static string GenerateBase32(HashAlgorithmKind hashingAlgorithm = HashAlgorithmKind.Sha1)
        {
            var bytes = new byte[(sbyte)hashingAlgorithm];
            _rng.GetBytes(bytes);
        ...
        {
            return ToBase32(Encoding.UTF8.GetBytes(input));
        }
        ...
        public static byte[] FromBase32(string input)
        {
            ...
            while (outputIndex < output.Length)
            {
                ...
                if (byteIndex < 0)
                {
                    throw new FormatException("Input isn't base32 string");
                }
